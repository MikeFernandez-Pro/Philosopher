...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/23 12:34:59 by user42            #+#    #+#             */
/*   Updated: 2020/10/23 12:35:00 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_eat(t_philo *philo)
{
	...
	if (sem_wait(global->keys) == 0 && sem_wait(philo->lock) == 0)
	{
		...
		philo->eat += 1;
		if (philo->eat == philo->global->nb_of_meals)
...
void	ft_core(t_global *global)
{
	...
	i = -1;
	if (pthread_create(&t_monitoring, NULL, ft_gbl_monitoring, global) != 0)
		return ;
	pthread_detach(t_monitoring);
